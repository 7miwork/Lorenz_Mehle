...
DIFFICULTY = {
    "easy": {
        "label": "Einfach",
        "desc": "Große Anlage ohne Haken – 200.000 € Startkapital",
        "start_cash": 200_000.0,
        "start_loan": 0.0,
        "income_mult": 1.25,
        "expense_mult": 0.85,
        "buy_chance_base": 0.55,
    },
    "medium": {
        "label": "Medium",
        "desc": "Gerade so ausreichend – 50.000 € Startkapital",
        "start_cash": 50_000.0,
        "start_loan": 0.0,
        "income_mult": 1.0,
        "expense_mult": 1.0,
        "buy_chance_base": 0.40,
    },
    "hard": {
        "label": "Schwer",
        "desc": "Mit 100.000 € Startkapital, aber 80.000 € Schulden – tilgen oder untergehen!",
        "start_cash": 100_000.0,
        "start_loan": 80_000.0,
        "income_mult": 0.75,
        "expense_mult": 1.2,
        "buy_chance_base": 0.30,
    },
}
...
ACHIEVEMENTS = [
    ...
    ("fiftymio",     "50-Millionär",            "Nettovermögen > 50 Mio €",
     lambda g: g.net_worth() >= 50_000_000),
    ("legend",       "Legende",                 "Nettovermögen > 100 Mio €",
     lambda g: g.net_worth() >= 100_000_000),
    ("investor",     "Investor",                "Aktienportfolio > 500k €",
     lambda g: g.stock_value() >= 500_000),
    ...
    ("fullhouse",    "Vollvermieter",           "4+ Immobilien gleichzeitig vermietet",
     lambda g: len(g.props) >= 4 and all(not p["vacant"] for p in g.props)),
    ("premium",      "Premium-Vermieter",       "Luxusmieter in einer Immobilie",
     lambda g: any(p.get("tenant")==3 for p in g.props)),
    ("survivor",     "Krisenüberlebender",      "Depression überlebt (NV über 0)",
     lambda g: getattr(g,"_survived_dep",False)),
    ("debtfree",     "Schuldenfrei",            "Alle Schulden getilgt (auf schwer startend)",
     lambda g: g.loan == 0 and len(g.nw_hist) > 3 and g.difficulty == "hard"),
    ("efficient",    "Effizient",               "10 Unternehmen auf Level 5+",
     lambda g: sum(1 for c in g.comps if c["level"] >= 5) >= 10),
    ("mogul",        "Immobilien-Mogul",         "Grundstückswert > 20 Mio €",
     lambda g: sum(p["price"] for p in g.props) >= 20_000_000),
    ("bourse",       "Börsenhai",               "Aktiengewinne > 1 Mio € realisiert (vereinfacht: Wert > 2 Mio)",
     lambda g: g.stock_value() >= 2_000_000),
    ("hardmode",     "Hardcore",                "Schwerer Modus + 1 Mio NV erreicht",
     lambda g: g.net_worth() >= 1_000_000 and g.difficulty == "hard"),
    ("combo",        "Alleskönner",             "5 Immobilien + 5 Firmen + 500k Aktien",
     lambda g: len(g.props)>=5 and len(g.comps)>=5 and g.stock_value()>=500_000),
]
...
# ─────────────────────────────────────────────────────
#  MARKT-FORMELN für Kauf-/Verkaufsverhalten
# ─────────────────────────────────────────────────────
def calc_buy_chance(sale_price, market_value, gs):
    """
    Berechnet die Wahrscheinlichkeit, dass ein Käufer eine Immobilie/Firma
    zum angebotenen Preis kauft.
    
    sale_price   : vom Spieler festgelegter Verkaufspreis
    market_value : aktueller Marktwert (berechnet)
    gs           : Spielzustand (für Wirtschaftsdaten)
    
    Rückgabe: (chance 0..1, message_key)
    """
    ...
    # Basis-Chance aus Schwierigkeitsgrad
    base = DIFFICULTY[gs.difficulty]["buy_chance_base"]
    
    # Preis-Faktor: je näher am Marktwert, desto besser
    if ratio <= 0.5:
        # Sehr günstig → fast sofortiger Verkauf
        price_factor = 2.5
    elif ratio <= 0.8:
        # Günstig → guter Verkauf
        price_factor = 2.0
    elif ratio <= 1.0:
        # Fairer Preis
        price_factor = 1.5 - (ratio - 0.8) * 2.5
    elif ratio <= 1.3:
        # Etwas teuer
        price_factor = 1.0 - (ratio - 1.0) * 2.0
    elif ratio <= 1.5:
        # Zu teuer
        price_factor = 0.4 - (ratio - 1.3) * 1.5
    else:
        # Viel zu teuer
        price_factor = 0.1
    ...
    sentiment_mod = (gs.sentiment - 50) / 100.0  # -0.5 bis +0.5
    sentiment_factor = 1.0 + sentiment_mod * 0.5  # 0.75 bis 1.25
    
    # Zufallsstreuung
    random_factor = 0.8 + random.random() * 0.4  # 0.8 bis 1.2
...
def get_price_message(msg_key):
    """Gibt eine kurze Nachricht über die Marktlage zurück."""
    messages = {
        ...
        "too_cheap": "Sensation! Schnäppchenjäger zugeschlagen!",
        "no_buyers": "Kein Interesse – Preis zu hoch für aktuelle Marktlage.",
    }
...
class GS:
    ...
    def __init__(self, difficulty="medium"):
        ...
        # Message-Puffer für Marktnachrichten (für UI-Kurznachrichten)
        self.market_messages : List[tuple] = []  # (msg, ms_start)
    ...
    def add_market_msg(self, msg):
        """Fügt eine Kurznachricht für das UI-Overlay hinzu."""
    ...
    def net_monthly(self):
        """Netto monatlich (nach Steuer, vor Cashflow-Auswirkung)."""
        diff = DIFFICULTY[self.difficulty]
    ...
    def name_of(self, item):
        """Gibt den Namen eines Gegenstandes zurück (Custom-Name falls vorhanden)."""
...
def make_prop(catalog_row):
    ...
    return {
        ...
        "custom_name": name,  # vom Spieler änderbar
        "price":     float(price),
        ...
        "listed":  False,      # auf dem Mietmarkt angeboten
        "tenant":  None,       # aktiver Mietertyp-Index oder None
        "contract_left": 0,    # verbleibende Monate
        "rent_hist": [],       # letzte 24 Monate Mieteinnahmen
        # Verkaufs-Marktplatz
        "for_sale": False,     # steht zum Verkauf?
        "sale_price": 0.0,     # vom Spieler festgelegter Preis
        "months_on_market": 0, # Monate auf dem Markt
    }
...
def process_sales(gs: GS):
    """
    Verarbeitet alle zum Verkauf stehenden Immobilien und Firmen.
    Berechnet die Kaufwahrscheinlichkeit und führt ggf. Verkäufe durch.
    """
    ...
    for p in gs.props:
        if p["for_sale"] and p["sale_price"] > 0:
            ...
            chance, msg_key = calc_buy_chance(p["sale_price"], p["price"], gs)
            
            # Kurznachricht zur Marktlage (nur bei bestimmten Schwellen)
            if p["months_on_market"] == 1 and chance < 0.1:
                gs.add_market_msg(f"{gs.name_of(p)}: {get_price_message('dead')}")
            elif p["months_on_market"] == 1 and chance >= 0.8:
                gs.add_market_msg(f"{gs.name_of(p)}: {get_price_message('hot')}")
            
            # Monatliche Verkaufschance
            if random.random() < chance:
                ...
                # Nachricht
                ratio = p["sale_price"] / p["price"]
                if ratio < 0.7:
                    gs.add_news(f"Schnäppchenjäger kauft {gs.name_of(p)} weit unter Wert!")
                    gs.add_log(f"{gs.name_of(p)} verkauft für {fmt(proceeds)} (Schnäppchen!)", "warn")
                elif ratio > 1.3:
                    gs.add_news(f"Luxuskauf! {gs.name_of(p)} zu Spitzenpreis verkauft.")
                    gs.add_log(f"{gs.name_of(p)} verkauft für {fmt(proceeds)} (Spitzenpreis!)", "good")
                else:
                    gs.add_news(f"{gs.name_of(p)} verkauft für {fmt(proceeds)}.")
                    gs.add_log(f"{gs.name_of(p)} verkauft für {fmt(proceeds)}", "info")
                
                gs.add_market_msg(f"{gs.name_of(p)}: {get_price_message('sold')}")
                
                # Immobilie aus dem Besitz entfernen (als verkauft markieren)
                p["for_sale"] = False
                p["vacant"] = True
                p["tenant"] = None
                p["listed"] = False
                # Wir entfernen sie aus der Liste am Ende der Schleife,
                # markieren sie stattdessen
                p["_sold"] = True
                
    # Verkaufte Immobilien entfernen
    gs.props = [p for p in gs.props if not p.get("_sold")]
    ...
    for c in gs.comps:
        if c["for_sale"] and c["sale_price"] > 0:
            ...
            if c["months_on_market"] == 1 and chance < 0.1:
                gs.add_market_msg(f"{gs.name_of(c)}: {get_price_message('dead')}")
            elif c["months_on_market"] == 1 and chance >= 0.8:
                gs.add_market_msg(f"{gs.name_of(c)}: {get_price_message('hot')}")
    ...
    gs.comps = [c for c in gs.comps if not c.get("_sold")]
...
def tick(gs: GS):
    ...
    # ── Immobilien ──
    for p in gs.props:
        # Nur wenn nicht zum Verkauf stehend (sonst keine Mieteinnahmen)
        if p["for_sale"]:
            # Wenn zum Verkauf angeboten, keine Miete (leer für Besichtigungen)
            if not p["vacant"]:
                # Mieter kündigen bei Verkaufsabsicht
                p["tenant"]        = None
                p["vacant"]        = True
                p["listed"]        = False
                p["contract_left"] = 0
        ...
        if p["listed"] and p["vacant"]:
            ...
            if random.random() < chance:
                ...
                gs.add_log(f"Neuer Mieter: {tname} in {gs.name_of(p)} ({months} Monate)", "good")
                gs.add_market_msg(f"{tname} in {gs.name_of(p)} eingezogen!")
    ...
    for c in gs.comps:
        if c["for_sale"]:
            continue  # kein Gewinn wenn zum Verkauf
        ...
        if random.random() < c["risk"] * 0.35:
            ...
            expenses += dmg
            gs.add_log(f"Schadenfall bei {gs.name_of(c)}: -{fmt(dmg)}", "bad")
        income   += c["profit"]
        expenses += c["maint"]
    ...
    # Depression überlebt? (nur wenn NV positiv)
    if gs.phase == "DEPRESSION" and gs.net_worth() > 0 and gs.cash > 0:
        gs._survived_dep = True

    # ── Bankrott-Prüfung (verschärft) ──
    # Wenn Bargeld tief im Minus UND Schulden sehr hoch
    # Oder wenn Nettovermögen negativ
    if gs.net_worth() < -10_000:
        gs.add_log("KRISE: Nettovermögen negativ! Handeln Sie schnell!", "bad")
        if gs.cash < -50_000 and gs.loan > 0:
            gs.add_news("Zahlungsunfähigkeit droht! Sofort handeln!")
            if gs.cash < -100_000:
                return "bankrott"
    
    # Zusätzlich: Wenn Schulden > 3x Nettovermögen (und NV positiv)
    if gs.loan > 0 and gs.net_worth() > 0 and gs.loan > gs.net_worth() * 3:
        gs.add_log("KRITISCHE VERSCHULDUNG! Gläubiger werden unruhig!", "bad")
        if gs.cash < -gs.loan * 0.1:
            return "bankrott"
    
    # Schwerer Modus: strengere Regeln
    if gs.difficulty == "hard":
        # Monatliche Mindesttilgung von 0.5% des Kredits verlangen
        if gs.loan > 0:
            min_payment = gs.loan * 0.005
            if gs.cash >= min_payment:
                gs.cash -= min_payment
                gs.loan = max(0, gs.loan - min_payment)
            else:
                gs.add_log("WARNUNG: Mindesttilgung nicht möglich! Kredit wächst!", "bad")
                gs.loan *= 1.01  # Schulden wachsen
                if gs.loan > gs.net_worth() * 2 and gs.cash < 0:
                    return "bankrott"
...
def _random_events(gs: GS):
    events = [
        ...
        # Schwere-Modus spezifische Events
        (0.012 if gs.difficulty=="hard" else 0.005, lambda: _ev_bank_run(gs)),
        (0.015 if gs.difficulty=="hard" else 0.008, lambda: _ev_credit_crunch(gs)),
    ]
...
def _ev_fire(gs):
    ...
    gs.add_log(f"Feuer in {gs.name_of(p)}! -{fmt(dmg)}", "bad")
...
def _ev_vacancy(gs):
    ...
    p["vacant"]        = True
    p["contract_left"] = 0
    gs.add_log(f"{tname} ausgezogen: {gs.name_of(p)} jetzt leer", "bad")
...
def _ev_lawsuit(gs):
    ...
    pen = c["val"]*0.07
    gs.cash -= pen
    gs.add_log(f"Klage vs {gs.name_of(c)}: -{fmt(pen)}", "bad")
...
def _ev_infra(gs):
    ...
    p = random.choice(gs.props)
    p["price"] *= 1.10
    gs.add_log(f"Stadtentwicklung: {gs.name_of(p)} +10%", "good")
...
def _ev_bank_run(gs):
    """Nur im schweren Modus: Bankenkrise - höhere Zinsen."""
    gs.base_rate *= 1.3
    gs.add_log("Bankenkrise! Zinsschock! Leitzins +30%", "bad")
    gs.add_news("Bankenkrise! Zinsen steigen rasant!")

def _ev_credit_crunch(gs):
    """Nur im schweren Modus: Kreditklemme."""
    if gs.loan > 0:
        extra = gs.loan * 0.02
        gs.cash -= extra
        gs.add_log(f"Kreditklemme! Zusätzliche Gebühren: -{fmt(extra)}", "bad")
        gs.add_news("Kreditklemme: Banken verlangen Sondertilgungen!")
...
class NameScreen:
    def __init__(self):
        ...
        # Schwierigkeitsgrad-Buttons mit Farbe
        btn_w = 150
        btn_h = 56
        spacing = 12
        ...
        self.diff_btns = {}
        diff_colors = {
            "easy":   (GREEN[0]//2, GREEN[1]//2, GREEN[2]//2),
        ...
        for i, key in enumerate(["easy", "medium", "hard"]):
            ...
            self.diff_btns[key] = Btn(bx2, H//2-18, btn_w, btn_h, 
    ...
    def draw(self, surf):
        ...
        # ── Titel (oben, viel Platz) ──
        txt(surf,"BUSINESS TYCOON PRO","title",GOLD, W//2,H//2-200,"center")
        txt(surf,"by Michael (其米）","md",MUTED, W//2,H//2-165,"center")
        
        # ── Name ──
        txt(surf,"Wie heißt du, Tycoon?","md",WHITE, W//2,H//2-120,"center")
        self.box.draw(surf)
        
        # ── Schwierigkeitsgrad (Abstand zur Box) ──
        txt(surf,"Schwierigkeitsgrad","sm",MUTED, W//2,H//2-45,"center")
        
        for key, btn in self.diff_btns.items():
            btn.draw(surf)
        
        # Rahmen um den ausgewählten Button
        sel_btn = self.diff_btns[self.diff_selected]
        border_col = {"easy": GREEN, "medium": YELLOW, "hard": RED}[self.diff_selected]
        box(surf, border_col,
            (sel_btn.rect.x-3, sel_btn.rect.y-3, sel_btn.rect.w+6, sel_btn.rect.h+6), 10, 2)
        
        # Beschreibung des gewählten Modus (unter den Buttons)
        ...
        y_leg = H//2+175
        legends = [
            ("Einfach",   "200k € Start, 25% mehr Einnahmen", GREEN),
            ("Medium",    "50k € Start, ausgewogen",          YELLOW),
            ("Schwer",    "100k € + 80k Schulden, Tilgung!",  RED),
        ]
        leg_x = W//2 - 240
        for i, (lbl, desc, col) in enumerate(legends):
            txt(surf, f"{lbl}:", "xs", col, leg_x, y_leg + i*18)
            txt(surf, desc, "xs", MUTED, leg_x + 50, y_leg + i*18)
        
        # Steuerungshinweis
        txt(surf, "Drücke ENTER oder klicke 'Spielen' um zu starten","xs",MUTED, W//2, H//2+230,"center")
...
class GameScreen:
    ...
    def _handle_main_click(self, mx, my):
        ...
        # ─ Topbar ─
        # Speed-Buttons (Werte leicht nach links verschoben)
    ...
    def _open_sell_prop(self):
        self._inputs = {"price": InputBox(0,0,200,34,"Verkaufspreis (€)", numeric=True)}
    ...
    def _open_rename_prop(self):
        self._inputs = {"name": InputBox(0,0,260,34,"Neuer Name", numeric=False)}
        self.modal = {"type":"rename_prop"}
        self._scroll = 0
    ...
    def _load_game(self):
        try:
            ...
            gs.year = data["year"]
            gs.phase = data["phase"]
            gs.phase_dur = data["phase_dur"]
            gs.base_rate = data["base_rate"]
            gs.inflation = data["inflation"]
            gs.gdp = data["gdp"]
            gs.unemp = data["unemp"]
            gs.sentiment = data["sentiment"]
            gs.reputation = data["reputation"]
            gs.achiev_done = set(data["achiev_done"])
            gs.nw_hist = data["nw_hist"]
            gs.cf_hist = data["cf_hist"]
            gs._survived_dep = data["_survived_dep"]
    ...
    def _handle_modal_click(self, mx, my):
        ...
        if mt == "buy_prop":
            ...
            for i, p in enumerate(gs.props):
                ...
                # Prüfe, ob die Immobilie bereits zum Verkauf steht
                if p["for_sale"]:
                    # Verkauf abbrechen-Button
                    if pygame.Rect(bx+mw-120, ry+14, 110, 28).collidepoint(mx,my):
                        p["for_sale"] = False
                        p["sale_price"] = 0
                    ...
                    # Preis eingeben und zum Verkauf anbieten
                    price_box_key = f"price_{i}"
                    if price_box_key not in self._inputs:
                        self._inputs[price_box_key] = InputBox(0, 0, 160, 32, "Preis (€)", numeric=True)
                    ib = self._inputs[price_box_key]
                    ib.rect = pygame.Rect(bx+20, ry+14, 160, 32)
                    ib.handle_click = lambda: None  # handled globally
                    ...
                    if pygame.Rect(bx+mw-120, ry+14, 110, 28).collidepoint(mx,my):
                        ...
                        if sale_price > 0:
                            ...
                            # Sofortige Rückmeldung zur Preisbewertung
                            chance, msg_key = calc_buy_chance(sale_price, p["price"], gs)
                            msg = get_price_message(msg_key)
                            if msg:
                                gs.add_market_msg(f"{gs.name_of(p)}: {msg}")
                                gs.add_log(f"Marktlage: {msg}", "info")
            ...
            for i, p in enumerate(gs.props):
                ...
                if not maxed and pygame.Rect(bx+mw-110, ry+20, 90, 30).collidepoint(mx,my):
                    if gs.cash >= cost:
                        ...
                        p["rent"]      *= 1.15
                        p["maint"]     *= 1.06
                        gs.add_log(f"Renoviert: {gs.name_of(p)} Lvl {p['level']}", "good")
                    self._close_modal()
                    return None
            ...
            for i, p in enumerate(gs.props):
                ...
                if p["vacant"] and not p["listed"] and not p["for_sale"]:
                ...
                elif not p["vacant"] and not p["for_sale"]:
                    if pygame.Rect(bx+mw-120, ry+20, 100, 28).collidepoint(mx,my):
                        tname = TENANT_TYPES[p["tenant"]][0] if p["tenant"] is not None else "Mieter"
                        p["tenant"]        = None
                        p["vacant"]        = True
                        p["listed"]        = False
                        p["contract_left"] = 0
                        # Strafe für vorzeitige Kündigung
                        penalty = p["rent"] * 2
                        gs.cash -= penalty
                ...
                elif p["vacant"] and p["listed"] and not p["for_sale"]:
                    if pygame.Rect(bx+mw-120, ry+20, 100, 28).collidepoint(mx,my):
                        p["listed"] = False
                        gs.add_log(f"{gs.name_of(p)} vom Mietmarkt genommen", "info")
                        self._close_modal()
                        return None

        elif mt == "rename_prop":
            self._inputs["name"].rect = pygame.Rect(bx+30, by+100, 260, 34)
            row_h = 50; y0 = by+150
            for i, p in enumerate(gs.props):
                ry = y0 + i*row_h - self._scroll
                if ry+row_h < by+40 or ry > by+mh-20: continue
                if pygame.Rect(bx+mw-130, ry+10, 110, 28).collidepoint(mx,my):
                    new_name = self._inputs["name"].text.strip()
                    if new_name:
                ...
                if c["for_sale"]:
                    ...
                    if pygame.Rect(bx+mw-120, ry+14, 110, 28).collidepoint(mx,my):
                        ...
                        if sale_price > 0:
                            ...
                            c["months_on_market"] = 0
                            gs.add_log(f"{gs.name_of(c)} für {fmt(sale_price)} angeboten", "info")
                            
                            chance, msg_key = calc_buy_chance(sale_price, c["val"], gs)
                            msg = get_price_message(msg_key)
                            if msg:
                                gs.add_market_msg(f"{gs.name_of(c)}: {msg}")
            ...
            for i, c in enumerate(gs.comps):
                ...
                if pygame.Rect(bx+mw-130, ry+10, 110, 28).collidepoint(mx,my):
                    ...
                    if new_name:
                        ...
                        gs.add_log(f"Firma umbenannt: {new_name}", "info")
                    self._close_modal()
                    return None
            ...
            for i, c in enumerate(gs.comps):
                ...
                if not maxed and pygame.Rect(bx+mw-110, ry+20, 90, 30).collidepoint(mx,my):
                    if gs.cash >= cost:
                        ...
                        c["maint"]       *= 1.08
                        gs.add_log(f"Firma erweitert: {gs.name_of(c)} Lvl {c['level']}", "good")
                    self._close_modal()
                    return None
            ...
            if pygame.Rect(bx2, by2, 120, 34).collidepoint(mx,my):
                ...
                # Im schweren Modus: geringerer Kreditrahmen
                max_l_mult = 0.4 if gs.difficulty == "hard" else 0.6
                max_l = max(0, gs.net_worth()*max_l_mult - gs.loan)
    ...
    def _draw_market_msgs(self):
        """Zeichnet kurze Marktnachrichten im unteren Bereich."""
        gs = self.gs
        now = pygame.time.get_ticks()
        y_start = H - 48
        x_start = 200
        
        # Alte Nachrichten entfernen (älter als 4 Sekunden)
    ...
    def _draw_topbar(self):
        ...
        txt(screen,f"Business Tycoon Pro [{DIFFICULTY[gs.difficulty]['label']}]","lg",GOLD, 10,22,"midleft")
    ...
    def _sidebar_rects(self):
        ...
        entries = [
            ...
            ("upg_prop",   "  Renovieren"),
            ("rent_prop",  "  Vermieten"),
            ("rename_prop","  Umbenennen"),
            ("__sec_comp__","Unternehmen"),
            ("buy_comp",   "  Gründen"),
            ("sell_comp",  "  Verkaufen"),
            ("upg_comp",   "  Erweitern"),
            ("rename_comp","  Umbenennen"),
    ...
    def _draw_sidebar(self):
        ...
        for key, ry, rh in self._sidebar_rects():
            ...
            if key.startswith("__"):
                ...
                # Hervorhebung für zum Verkauf stehende Objekte
                has_sales = (key == "sell_prop" and
                            any(p["for_sale"] for p in self.gs.props))
                bg = (40,35,20) if has_sales else ((60,40,15) if hi else PANEL2)
                box(screen, bg, (4,ry,182,rh), 5)
                bc = YELLOW if has_sales else (YELLOW if hi else PANEL2)
                if has_sales or hi:
                    box(screen, YELLOW if hi else ORANGE, (4,ry,182,rh), 5, 1)
                txt(screen, label, "sm", ORANGE if has_sales else (YELLOW if hi else WHITE), 
                    12, ry+rh//2, "midleft")
        ...
        if gs.props:
            ...
            for p in gs.props:
                ...
                if p["vacant"] and not p["listed"] and not p["for_sale"]:
                    ...
                    col2, status2 = ORANGE, "ZU VK"
                else:
                    ti = p.get("tenant")
                    col2 = GREEN
                    status2 = TENANT_TYPES[ti][0][:8] if ti is not None else "Vermietet"
                box(screen, col2, (px2, cy3+4, pw2, 20), 4)
                txt(screen, gs.name_of(p)[:7], "xs", BG, px2+4, cy3+14, "midleft")
                px2 += pw2 + 4
            cy3 += bar_h + 6
            ch3 -= bar_h + 6
            
        # Verkaufs-Status
        for_sale_items = [p for p in gs.props if p["for_sale"]] + [c for c in gs.comps if c["for_sale"]]
        if for_sale_items:
            bar_h2 = 20
            box(screen, (40,30,15), (x+pad, cy3, w-pad*2, bar_h2), 6)
            txt(screen, f"Angebote: {len(for_sale_items)} Objekt(e) auf dem Markt", "xs", ORANGE, 
                x+pad+8, cy3+bar_h2//2, "midleft")
            cy3 += bar_h2 + 4
            ch3 -= bar_h2 + 4
    ...
    def _draw_modal(self):
        ...
        elif mt == "sell_prop": self._draw_m_sell_prop(bx,by,mw,mh)
        elif mt == "upg_prop":  self._draw_m_upg_prop(bx,by,mw,mh)
        elif mt == "rent_prop": self._draw_m_rent_prop(bx,by,mw,mh)
        elif mt == "rename_prop": self._draw_m_rename_prop(bx,by,mw,mh)
        elif mt == "buy_comp":  self._draw_m_buy_comp(bx,by,mw,mh)
        elif mt == "sell_comp": self._draw_m_sell_comp(bx,by,mw,mh)
        elif mt == "rename_comp": self._draw_m_rename_comp(bx,by,mw,mh)
    ...
    def _draw_m_sell_prop(self, bx, by, mw, mh):
        ...
        txt(screen,"Preis festlegen → Käufer suchen (Marktformel)  |  Zum Verkauf: KEINE Mieteinnahmen!",
            "xs", RED, bx+16, by+40)
        if not gs.props:
        ...
        for i,p in enumerate(gs.props):
            ...
            bg = (40,35,20) if p["for_sale"] else PANEL2
            bc = ORANGE if p["for_sale"] else BORDER
            box(view,bg,(8,ry,mw-16,row_h-4),7)
            box(view,bc,(8,ry,mw-16,row_h-4),7,1)
            
            txt(view,f"{gs.name_of(p)}  (Lvl {p['level']})","md",WHITE,18,ry+10)
            txt(view,f"Marktwert: {fmt(p['price'])}  |  Netto: {fmt(p['rent']-p['maint'])}/Monat",
                "xs", MUTED, 18, ry+32)
            
            if p["for_sale"]:
                txt(view,f"ANGEBOTEN für: {fmt(p['sale_price'])}  |  Monate auf Markt: {p['months_on_market']}",
                    "xs", ORANGE, 18, ry+50)
                box(view,RED,(mw-128,ry+14,110,28),6)
                txt(view,"Angebot stornieren","xs",WHITE,mw-73,ry+28,"center")
                
                # Preis-Markt-Einschätzung anzeigen
                chance, msg_key = calc_buy_chance(p["sale_price"], p["price"], gs)
                msg = get_price_message(msg_key)
                if msg:
                    txt(view, msg, "xs", CYAN, 240, ry+50)
            else:
                price_box_key = f"price_{i}"
                if price_box_key not in self._inputs:
                    self._inputs[price_box_key] = InputBox(0, 0, 160, 32, "Preis (€)", numeric=True)
                ib = self._inputs[price_box_key]
                ib.rect = pygame.Rect(18, ry+44, 140, 28)
                ib.draw(view)
                
                # Sofortige Preisbewertung anzeigen, wenn Preis eingegeben
                if ib.text:
                    try:
                        test_price = float(ib.text)
                        if test_price > 0:
                            chance, msg_key = calc_buy_chance(test_price, p["price"], gs)
                            msg = get_price_message(msg_key)
                            if msg:
                                txt(view, msg, "xs", CYAN if chance >= 0.4 else RED, 170, ry+52)
                    except:
                        pass
                
                box(view,GREEN,(mw-128,ry+14,110,28),6)
                txt(view,"Anbieten","sm",BG,mw-73,ry+28,"center")
            
            if p.get("vacant"):
                txt(view,"(Leerstand)","xs",RED,12,ry+10)
    ...
    def _draw_m_upg_prop(self, bx, by, mw, mh):
        ...
        for i,p in enumerate(gs.props):
            ...
            cost   = p["price"]*0.12
            maxed  = p["level"] >= p["lvl_max"]
            can    = gs.cash >= cost and not maxed and not p["for_sale"]
            box(view,PANEL2,(8,ry,mw-16,row_h-4),7)
            box(view,BORDER,(8,ry,mw-16,row_h-4),7,1)
            txt(view,gs.name_of(p),"md",WHITE,18,ry+10)
            ...
            if p["for_sale"]:
                ...
                txt(view,"Im Verkauf","xs",MUTED,mw-67,ry+36,"center")
            elif maxed:
                box(view,BORDER,(mw-112,ry+22,90,28),6)
    ...
    def _draw_m_rent_prop(self, bx, by, mw, mh):
        ...
        for i, p in enumerate(gs.props):
            ...
            # Für Verkauf angeboten? → keine Vermietung möglich
            if p["for_sale"]:
                box(view, (40,30,15), (8, ry, mw-16, row_h-6), 8)
                box(view, ORANGE, (8, ry, mw-16, row_h-6), 8, 1)
                txt(view, f"{gs.name_of(p)}  Lvl {p['level']}", "lg", ORANGE, 22, ry+10)
                txt(view, "ZUM VERKAUF ANGEBOTEN – keine Vermietung", "sm", RED, 22, ry+40)
                continue
            ...
            txt(view, f"{gs.name_of(p)}  Lvl {p['level']}", "lg", WHITE, 22, ry+10)
            ...
            if p["vacant"] and not p["listed"]:
                x_off = 400
                txt(view, "Mögl. Mieter:", "xs", MUTED, x_off, ry+10)
            ...
            if p["vacant"] and not p["listed"]:
                ...
                txt(view, "Kündigen (-2M)", "xs", WHITE, mw-74, ry+35, "center")

    def _draw_m_rename_prop(self, bx, by, mw, mh):
        gs = self.gs
        txt(screen,"Immobilie umbenennen","lg",GOLD,bx+16,by+16)
        ib = self._inputs["name"]
        ib.rect = pygame.Rect(bx+30, by+60, 260, 34)
        ib.draw(screen)
        txt(screen,"Wähle eine Immobilie und klicke auf 'Umbenennen':","xs",MUTED,bx+30,by+100)
        if not gs.props:
            txt(screen,"Keine Immobilien vorhanden.","md",MUTED,bx+30,by+130)
            return
        row_h = 50
        view = screen.subsurface(pygame.Rect(bx, by+120, mw, mh-130))
        for i,p in enumerate(gs.props):
            ry = i*row_h - self._scroll
            if ry+row_h<0 or ry>mh-130: continue
            box(view,PANEL2,(8,ry,mw-16,row_h-4),7)
            box(view,BORDER,(8,ry,mw-16,row_h-4),7,1)
            txt(view,f"{gs.name_of(p)}  (aktuell: {p.get('custom_name',p['name'])})","sm",WHITE,18,ry+16)
            box(view,ACCENT,(mw-130,ry+10,110,28),6)
            txt(view,"Umbenennen","xs",WHITE,mw-75,ry+24,"center")
    ...
    def _draw_m_sell_comp(self, bx, by, mw, mh):
        ...
        txt(screen,"Preis festlegen → Käufer suchen (Marktformel)  |  Zum Verkauf: KEINE Gewinne!",
            "xs", RED, bx+16, by+40)
        if not gs.comps:
            txt(screen,"Keine Unternehmen vorhanden.","md",MUTED,bx+16,by+80)
            return
        row_h = 80; y0 = by+56
        view = screen.subsurface(pygame.Rect(bx, by+50, mw, mh-60))
        for i,c in enumerate(gs.comps):
            ...
            bg = (40,35,20) if c["for_sale"] else PANEL2
            bc = ORANGE if c["for_sale"] else BORDER
            box(view,bg,(8,ry,mw-16,row_h-4),7)
            box(view,bc,(8,ry,mw-16,row_h-4),7,1)
            
            txt(view,f"{gs.name_of(c)}  (Lvl {c['level']})","md",WHITE,18,ry+10)
            txt(view,f"Marktwert: {fmt(c['val'])}  |  Gewinn: {fmt(c['profit'])}/Monat",
                "xs", MUTED, 18, ry+32)
            
            if c["for_sale"]:
                txt(view,f"ANGEBOTEN für: {fmt(c['sale_price'])}  |  Monate: {c['months_on_market']}",
                    "xs", ORANGE, 18, ry+50)
                box(view,RED,(mw-128,ry+14,110,28),6)
                txt(view,"Stornieren","xs",WHITE,mw-73,ry+28,"center")
            else:
                price_box_key = f"price_c_{i}"
                if price_box_key not in self._inputs:
                    self._inputs[price_box_key] = InputBox(0, 0, 160, 32, "Preis (€)", numeric=True)
                ...
                if ib.text:
                    try:
                        test_price = float(ib.text)
                        if test_price > 0:
                            chance, msg_key = calc_buy_chance(test_price, c["val"], gs)
                            msg = get_price_message(msg_key)
                            if msg:
                                txt(view, msg, "xs", CYAN if chance >= 0.4 else RED, 170, ry+52)
                    except:
    ...
    def _draw_m_rename_comp(self, bx, by, mw, mh):
        ...
        ib.rect = pygame.Rect(bx+30, by+60, 260, 34)
        ib.draw(screen)
        txt(screen,"Wähle eine Firma und klicke auf 'Umbenennen':","xs",MUTED,bx+30,by+100)
        if not gs.comps:
            txt(screen,"Keine Unternehmen vorhanden.","md",MUTED,bx+30,by+130)
            return
        row_h = 50
        view = screen.subsurface(pygame.Rect(bx, by+120, mw, mh-130))
        for i,c in enumerate(gs.comps):
            ...
            box(view,PANEL2,(8,ry,mw-16,row_h-4),7)
            box(view,BORDER,(8,ry,mw-16,row_h-4),7,1)
            txt(view,f"{gs.name_of(c)}  (aktuell: {c.get('custom_name',c['name'])})","sm",WHITE,18,ry+16)
            box(view,ACCENT,(mw-130,ry+10,110,28),6)
            txt(view,"Umbenennen","xs",WHITE,mw-75,ry+24,"center")
    ...
    def _draw_m_upg_comp(self, bx, by, mw, mh):
        ...
        for i,c in enumerate(gs.comps):
            ...
            cost  = c["val"]*0.15
            maxed = c["level"] >= c["lvl_max"]
            can   = gs.cash >= cost and not maxed and not c["for_sale"]
            box(view,PANEL2,(8,ry,mw-16,row_h-4),7)
            box(view,BORDER,(8,ry,mw-16,row_h-4),7,1)
            txt(view,gs.name_of(c),"md",WHITE,18,ry+10)
    ...
    def _draw_m_loan(self, bx, by, mw, mh):
        ...
        if gs.difficulty == "hard":
            txt(screen,"Achtung: Schwerer Modus – geringerer Rahmen, strengere Tilgung!","xs",RED,bx+16,by+170)
        ...
        txt(screen,"Warnung: Hohe Schulden führen zu Bankrott!","xs",RED,bx+16,by+150)
    ...
    def _draw_m_repay(self, bx, by, mw, mh):
        ...
        if gs.difficulty == "hard":
            txt(screen,"Empfohlen: so schnell wie möglich tilgen (Zinsbelastung!)","xs",ORANGE,bx+16,by+220)
        ib = self._inputs["amount"]
        ib.rect = pygame.Rect(bx+30, by+128, 220, 34)
        ib.draw(screen)
...
class BankruptScreen:
    ...
    def draw(self, surf):
        ...
        txt(surf,f"Schwierigkeit: {DIFFICULTY[self.gs.difficulty]['label']}","md",MUTED,W//2,H//2+50,"center")
        self.btn.draw(surf)
        ...
        for ev in pygame.event.get():
            ...
            if state == "name":
                ...
                if result:
                    ...
                    gs.add_log(f"Willkommen, {gs.name}! Schwierigkeit: {diff_label}", "info")
                    gs.add_log(f"Startkapital: {fmt(gs.cash)}  |  Schulden: {fmt(gs.loan)}", "info")
                    gs.add_news("Spielstart! Viel Erfolg beim Investieren.")
                    if difficulty == "hard":
                        gs.add_news("Du startest mit Schulden! Tilge sie so schnell wie möglich!")
