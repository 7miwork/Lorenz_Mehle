...
def draw_extra_fighter(surf, cx, cy, color, facing, anim, attacking, blocking, special, hurt, char_type):
    if hurt: color = (255, 100, 100)
    head_bob = math.sin(anim * 0.32) * 3 if not attacking else 0
    head_r = 17
    head_y = cy - 82 + head_bob
    body_top = int(head_y) + head_r
    body_bot = cy - 12
    ...
    if char_type == "viking":
        pygame.draw.arc(surf, SILVER, (cx - 28, int(head_y) - 30, 56, 28), math.pi, 2 * math.pi, 3)
        pygame.draw.line(surf, SILVER, (cx - 18, int(head_y) - 18), (cx - 36, int(head_y) - 32), 3)
        pygame.draw.line(surf, SILVER, (cx + 18, int(head_y) - 18), (cx + 36, int(head_y) - 32), 3)
        pygame.draw.line(surf, BROWN, (cx - 10, body_top + 18), (cx + 10, body_top + 18), 4)
    elif char_type == "boxer":
        pygame.draw.line(surf, RED, (cx - head_r + 2, int(head_y) - 6), (cx + head_r - 2, int(head_y) - 6), 4)
        pygame.draw.line(surf, color, (cx - 12, body_top + 12), (cx + 12, body_top + 12), 3)
    elif char_type == "soldier":
        pygame.draw.rect(surf, GREEN, (cx - 19, int(head_y) - 20, 38, 10), border_radius=4)
        pygame.draw.line(surf, GREEN, (cx - 12, body_top + 16), (cx + 12, body_top + 16), 4)
    elif char_type == "devil":
        pygame.draw.polygon(surf, DARK_RED, [(cx - 12, int(head_y) - 15), (cx - 24, int(head_y) - 34), (cx - 4, int(head_y) - 22)])
        pygame.draw.polygon(surf, DARK_RED, [(cx + 12, int(head_y) - 15), (cx + 24, int(head_y) - 34), (cx + 4, int(head_y) - 22)])
        pygame.draw.line(surf, DARK_RED, (cx - 9, body_top + 16), (cx + 9, body_top + 16), 3)

    pygame.draw.line(surf, color, (cx, body_top), (cx, body_bot), 4)
    leg_swing = math.sin(anim * 0.35) * 20
    if attacking: leg_swing = 15 * facing
    lk = (cx - 13, body_bot + 23); lf = (cx - 18, cy + 5)
    rk = (cx + 13, body_bot + 23 - int(leg_swing * 0.3)); rf = (cx + 18 + int(leg_swing * 0.3), cy + 5)
    pygame.draw.line(surf, color, (cx, body_bot), lk, 3); pygame.draw.line(surf, color, lk, lf, 3)
    pygame.draw.line(surf, color, (cx, body_bot), rk, 3); pygame.draw.line(surf, color, rk, rf, 3)

    reach = 38 if char_type == "boxer" else 48
    if attacking:
        ax = cx + facing * 32; ay = arm_top + int(math.sin(anim * 0.9) * 8)
        pygame.draw.line(surf, color, (cx - 8, arm_top + 4), (ax, ay + 4), 3)
        pygame.draw.line(surf, color, (cx + 8, arm_top + 4), (ax, ay - 4), 3)
        if char_type == "boxer":
            pygame.draw.circle(surf, RED, (ax + facing * 12, ay), 9)
        elif char_type == "viking":
            pygame.draw.line(surf, BROWN, (ax, ay), (ax + facing * reach, ay - 10), 5)
            pygame.draw.circle(surf, GRAY, (ax + facing * (reach + 8), ay - 12), 12)
        elif char_type == "soldier":
            pygame.draw.line(surf, DARK_GR, (ax, ay), (ax + facing * 55, ay), 5)
            pygame.draw.line(surf, GRAY, (ax + facing * 15, ay - 5), (ax + facing * 35, ay - 5), 4)
        else:
            tip = (ax + facing * 55, ay - 6)
            pygame.draw.line(surf, DARK_RED, (ax, ay), tip, 4)
            pygame.draw.line(surf, DARK_RED, tip, (tip[0], tip[1] - 16), 3)
            pygame.draw.line(surf, DARK_RED, tip, (tip[0], tip[1] + 16), 3)
    elif blocking:
        ax = cx + facing * 26
        pygame.draw.line(surf, color, (cx - 8, arm_top), (ax, arm_top + 10), 3)
        pygame.draw.line(surf, color, (cx + 8, arm_top), (ax, arm_top - 10), 3)
        shield_col = GRAY if char_type != "boxer" else RED
        pygame.draw.circle(surf, shield_col, (ax, arm_top), 15, 3)
    else:
        pygame.draw.line(surf, color, (cx - 8, arm_top), (cx - 18, arm_top + 22), 3)
        pygame.draw.line(surf, color, (cx + 8, arm_top), (cx + facing * 18, arm_top + 22), 3)

    if special:
        fx = cx + facing * 52
        effect_col = {"viking": GOLD, "boxer": RED, "soldier": YELLOW, "devil": ORANGE}[char_type]
        for i in range(8):
            angle = anim * 0.3 + i * (math.pi / 4)
            ex = fx + int(math.cos(angle) * (22 + i % 2 * 12))
            ey = int(head_y) + 20 + int(math.sin(angle) * 28)
            pygame.draw.circle(surf, effect_col, (ex, ey), 4)
...
def draw_background(surf, tick):
    ...
    star_random = random.Random(42)
    for _ in range(60):
        sx = star_random.randint(0, W); sy = star_random.randint(0, 180)
...
class Fighter:
    ...
    def __init__(self, char_type, x, facing):
        ...
        stats = {
            "samurai": (110, 16, 0.20, 35, BLUE, "SAMURAI", 95, 120, 180),
            "ninja":   (100, 17, 0.20, 28, RED, "NINJA", 90, 140, 160),
            "viking":  (125, 19, 0.25, 42, BROWN, "VIKINGER", 100, 135, 200),
            "boxer":   (115, 18, 0.15, 32, TAN, "BOXER", 80, 95, 140),
            "soldier": (105, 15, 0.20, 30, GREEN, "SOLDAT", 120, 230, 190),
            "devil":   (110, 17, 0.18, 38, DARK_RED, "TEUFEL", 105, 165, 180),
        }
        self.max_hp, self.attack, self.defense, self.sp_dmg, self.color, self.name, self.attack_reach, self.special_reach, self.special_cd_max = stats[char_type]
        self.hp = self.max_hp
    ...
    def do_attack(self):
        if self.attack_cd <= 0 and not self.attacking:
            self.attacking = True; self.attack_timer = 25; self.hit_registered = False
            self.attack_cd = 25 if self.char_type == "boxer" else 35
    ...
    def do_special(self):
        if self.special_cd <= 0 and not self.special:
            self.special = True; self.special_timer = 40; self.hit_registered = False; self.special_cd = self.special_cd_max
            if self.char_type == "boxer":
                self.hp = min(self.max_hp, self.hp + 8)
                self.block_stamina = min(self.max_block_stamina, self.block_stamina + 25)
    ...
    def draw(self, surf):
        ...
        elif self.char_type == "ninja": draw_ninja(surf, int(self.x), int(self.y), self.color, self.facing, self.anim_tick, self.attacking, self.blocking, self.special, self.hurt)
        else: draw_extra_fighter(surf, int(self.x), int(self.y), self.color, self.facing, self.anim_tick, self.attacking, self.blocking, self.special, self.hurt, self.char_type)

# ═══════════════════════════════════════════════════════════════════════════════
#  VERBESSERTE KI (Smarter & Unregelmäßiger)
# ═══════════════════════════════════════════════════════════════════════════════
...
class SimpleAI:
    def __init__(self, fighter, target):
        ...
        self.mode = "aggressive" # Modi: aggressive, defensive, tactical

    def update(self):
        f = self.fighter; t = self.target
        dist = abs(f.x - t.x)
        
        # Modus-Wechsel zur Unregelmäßigkeit
        if random.random() < 0.01:
            self.mode = random.choice(["aggressive", "defensive", "tactical"])

        self.think_timer -= 1
        if self.think_timer <= 0:
            # KI denkt jetzt schneller (10-25 statt 15-40)
            self.think_timer = random.randint(10, 25)
            roll = random.random()

            if dist < 100:
                if self.mode == "aggressive":
                    if roll < 0.6: self.action = "attack"
                    elif roll < 0.8: self.action = "special" if f.special_cd <= 0 else "attack"
                    else: self.action = "block"
                elif self.mode == "defensive":
                    if roll < 0.5: self.action = "block"
                    elif roll < 0.8: self.action = "retreat"
                    else: self.action = "attack"
                else: # Tactical
                    if roll < 0.3: self.action = "attack"
                    elif roll < 0.6: self.action = "block"
                    elif roll < 0.8: self.action = "special" if f.special_cd <= 0 else "retreat"
                    else: self.action = "retreat"
            else:
                self.action = "approach" if roll < 0.9 else "idle"
        ...
        if self.action == "approach":
            ...
            if dist < f.attack_reach: atk = True
            else: (right if f.x < t.x else None) or (left if f.x > t.x else None) 
        elif self.action == "block": blk = True
        elif self.action == "special":
            if dist < f.special_reach and f.special_cd <= 0: spc = True

        # REAKTIONS-LOGIK (Härtes Gegner-Gefühl)
        # Wenn Spieler angreift, sofort blocken oder springen
        if (t.attacking or t.special) and dist < 120:
            if random.random() < 0.4:
                blk = True
                atk = False # Blocken geht vor Angreifen
            if random.random() < 0.2 and f.on_ground:
                jump = True
...
def draw_hud(surf, p1, p2, tick):
    for p, x_off, is_right in [(p1, 20, False), (p2, W - 300, True)]:
        ...
        sp_pct = 1.0 - (p.special_cd / p.special_cd_max) if p.special_cd > 0 else 1.0
...
def char_select_screen():
    chars = ["samurai", "ninja", "viking", "boxer", "soldier", "devil"]; selected = 0; tick = 0
    preview = {
        "samurai": (BLUE, "SAMURAI"),
        "ninja": (RED, "NINJA"),
        "viking": (BROWN, "VIKINGER"),
        "boxer": (TAN, "BOXER"),
        "soldier": (GREEN, "SOLDAT"),
        "devil": (DARK_RED, "TEUFEL"),
    }
    while True:
        tick += 1
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                if event.key in (pygame.K_LEFT, pygame.K_a): selected = (selected - 1) % len(chars)
                if event.key in (pygame.K_RIGHT, pygame.K_d): selected = (selected + 1) % len(chars)
                if event.key in (pygame.K_UP, pygame.K_w): selected = (selected - 3) % len(chars)
                if event.key in (pygame.K_DOWN, pygame.K_s): selected = (selected + 3) % len(chars)
        ...
        card_w, card_h = 170, 170
        for i, ch in enumerate(chars):
            col = i % 3; row = i // 3
            x = W // 2 - 280 + col * 195
            y = 115 + row * 185
            cx = x + card_w // 2; is_sel = (i == selected)
            border_col = GOLD if is_sel else GRAY; bob = int(math.sin(tick * 0.05) * 6) if is_sel else 0
            card_rect = pygame.Rect(x, y - bob, card_w, card_h)
            ...
            color, name = preview[ch]
            if ch == "samurai": draw_samurai(screen, cx, y + 120 - bob, color, 1, tick, False, False, False, False)
            elif ch == "ninja": draw_ninja(screen, cx, y + 120 - bob, color, 1, tick, False, False, False, False)
            else: draw_extra_fighter(screen, cx, y + 120 - bob, color, 1, tick, False, False, False, False, ch)
            name_lbl = font_small.render(name, True, color)
            screen.blit(name_lbl, (cx - name_lbl.get_width() // 2, y + 135 - bob))
        pygame.display.flip(); clock.tick(60)

def fight(player_char):
    possible_enemies = ["samurai", "ninja", "viking", "boxer", "soldier", "devil"]
    possible_enemies.remove(player_char)
    ai_char = random.choice(possible_enemies)
        ...
        if not round_over:
            ...
            for attacker, defender in [(p1, p2), (p2, p1)]:
                if (attacker.attacking or attacker.special) and not attacker.hit_registered:
                    reach = attacker.special_reach if attacker.special else attacker.attack_reach
